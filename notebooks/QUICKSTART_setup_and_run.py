...
# MAGIC
# MAGIC Works even if `sql/` files are missing on workspace (embedded SQL fallback).
...
def load_sql_file(filename: str) -> str:
    ...
    if filename in EMBEDDED_SQL:
        print(
            f"Using embedded SQL for {filename}. "
            "Git → Pull when you can to sync sql/ folder."
        )
        return EMBEDDED_SQL[filename]
    raise FileNotFoundError(
        f"sql/{filename} not found. Tried:\n  " + "\n  ".join(tried[:8])
    )
