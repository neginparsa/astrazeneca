...
-- MAGIC
-- MAGIC Synthetic **AstraZeneca-themed** specialty pharmacy data (portfolio / learning only — not official AZ data).
-- MAGIC
-- MAGIC | Layer | ~Volume |
-- MAGIC |-------|---------|
-- MAGIC | Patients | **1,500** |
-- MAGIC | Claims | **~18,000** |
-- MAGIC | Brands | Tagrisso, Imfinzi, Farxiga, Fasenra, Lynparza, Ultomiris, Calquence, Tezspire, Brilinta, Symbicort |
-- MAGIC
-- MAGIC Catalog: **main**
...
SELECT
  ...
  END AS days_supply,
  CAST(
    pr.avg_wac_per_fill * (0.85 + (ABS(HASH(p.patient_id, f.fill_num)) % 30) / 100.0)
    AS DECIMAL(12, 2)
  ) AS paid_amount,
  pr.therapy_code,
  pr.brand_name,
  pr.therapeutic_area,
  pr.molecule,
  'az/specialty_pharmacy/claims' AS source_file,
...
SELECT UUID() AS rx_id, p.patient_id, pr.therapy_code, pr.brand_name,
  ...
  CASE ABS(HASH(p.patient_id, s.ship_num)) % 5
    ...
    WHEN 3 THEN 'PRIOR_AUTH_HOLD' ELSE 'SHIPPED'
  END AS hub_status,
...
SELECT UUID() AS auth_id, patient_id, pr.therapy_code, pr.brand_name,
  CASE ABS(HASH(patient_id)) % 6
    ...
    WHEN 3 THEN 'APPROVED' WHEN 4 THEN 'APPEAL_APPROVED' ELSE 'DENIED'
  END AS status,
...
FROM patients
JOIN main.gold.az_product_dim pr ON ABS(HASH(patient_id)) % 10 = ABS(HASH(pr.therapy_code)) % 10;
...
SELECT UUID() AS interaction_id, p.patient_id,
  ...
  CASE ABS(HASH(p.patient_id, t.touch_num)) % 5
    ...
    WHEN 3 THEN 'PA_SUPPORT' ELSE 'SIDE_EFFECT_TRIAGE'
  END AS outcome,
...
SELECT claim_id,
  SHA2(CAST(patient_id AS STRING), 256) AS patient_token,
  therapy_code, brand_name, therapeutic_area, molecule,
  fill_date, days_supply, paid_amount,
  SHA2(CONCAT('npi:', provider_npi), 256) AS provider_token,
...
SELECT SHA2(CONCAT(claim_id, CAST(fill_date AS STRING), 'CLAIM'), 256) AS event_id,
  SHA2(CAST(patient_id AS STRING), 256) AS patient_token,
  'CLAIM_FILL' AS event_type, therapy_code, brand_name, therapeutic_area,
...
SELECT patient_token, therapy_code, brand_name, therapeutic_area, CURRENT_DATE() AS as_of_date,
  ...
  CAST(4 + ABS(HASH(patient_token)) % 10 AS INT) AS fills_last_90d,
  pa_denial_count AS pa_denials_last_180d,
  hub_interactions_30d AS crm_outreach_last_30d,
...
UNION ALL SELECT 'gold.daily_franchise_metrics', COUNT(*) FROM main.gold.daily_franchise_metrics
UNION ALL SELECT 'gold.patient_journey', COUNT(*) FROM main.gold.patient_journey
UNION ALL SELECT 'gold.az_product_dim', COUNT(*) FROM main.gold.az_product_dim;

-- COMMAND ----------

SELECT therapeutic_area, brand_name, SUM(active_patients) AS fills, ROUND(SUM(total_paid_amount), 0) AS paid
FROM main.gold.daily_therapy_metrics
GROUP BY therapeutic_area, brand_name
ORDER BY paid DESC;

-- COMMAND ----------

-- MAGIC %md
-- MAGIC ## Done — import dashboards
-- MAGIC 1. SQL warehouse **Running**
-- MAGIC 2. Import `dashboards/astrazeneca_executive.lvdash.json`
-- MAGIC 3. Import `dashboards/astrazeneca_franchise.lvdash.json`
-- MAGIC 4. Import `dashboards/astrazeneca_patient_journey.lvdash.json`
